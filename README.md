...
这是一个 Node.js 实现的中间层服务器，用于在客户端和后端 API 管理服务器之间添加动态变量处理、内容缓存、功能扩展等。用于实现方便AI自身使用的时钟，日历，记事本功能。形成跨对话、跨客户端记忆同步，并构建持久化Base64编译缓存。
...
**注意**: 系统提示词转换、全局上下文转换和图片转译是自动进行的，不需要特定变量来触发。
